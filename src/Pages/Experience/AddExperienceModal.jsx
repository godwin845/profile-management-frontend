          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-6">
              ...
              <div className="pt-4 border-t border-white/20 dark:border-slate-800/50">
                ...
                <label className="relative flex items-center gap-4 cursor-pointer group p-4 rounded-2xl bg-white/50 dark:bg-slate-800/50 backdrop-blur-sm border border-white/30 dark:border-slate-700/50 hover:shadow-xl hover:scale-[1.02] hover:-translate-y-0.5 transition-all duration-300">
                  ...
                  <div className="relative">
                    ...
                    <div className="w-12 h-6 bg-slate-200 peer-focus:ring-4 peer-focus:ring-indigo-300/50 rounded-full peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-0.5 after:left-0.5 after:bg-white after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-indigo-600 shadow-lg" />
                  </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-6 border-t border-white/20 dark:border-slate-800/50">
              ...
              <button
                type="button"
                onClick={onClose}
                className="flex-1 px-8 py-4 bg-white/60 dark:bg-slate-800/60 text-slate-700 dark:text-slate-300 font-semibold rounded-2xl border border-white/40 dark:border-slate-700/50 shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all"
              >
                Cancel
              </button>
              ...
              <button
                type="submit"
                className="flex-1 px-8 py-4 bg-linear-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white font-bold rounded-2xl shadow-2xl hover:shadow-3xl hover:scale-[1.02] transition-all"
              >
...
const InputField = ({ label, name, value, onChange, placeholder, required }) => (
  <div className="space-y-2">
    ...
    <input
      ...
      required={required}
      className="w-full px-4 py-3.5 bg-white/70 dark:bg-slate-800/70 backdrop-blur-sm border border-slate-200/60 dark:border-slate-700/60 rounded-2xl text-sm shadow-lg focus:ring-4 focus:ring-indigo-500/30 focus:border-indigo-500/50 transition-all"
    />
  </div>
);
...
const DateInput = ({ label, name, value, onChange, required, disabled }) => (
  <div className="space-y-2">
    ...
    <input
      ...
      required={required}
      disabled={disabled}
      className={`w-full px-4 py-3.5 bg-white/70 dark:bg-slate-800/70 border border-slate-200/60 dark:border-slate-700/60 rounded-2xl text-sm shadow-lg focus:ring-4 focus:ring-indigo-500/30 focus:border-indigo-500/50 transition-all ${
